...
BoomaInput::BoomaInput(ConfigOptions* opts, bool* isTerminated):
    ...
    _rfGain = new HGain<int16_t>(_rfSplitter->Consumer(), opts->GetRfGain(), BLOCKSIZE, _rfGainProbe);
}
...
bool BoomaInput::SetInputReader(ConfigOptions* opts) {
    ...
    switch( opts->GetInputSourceType() ) {
        case AUDIO_DEVICE:
            ...
            _inputReader = new BoomaInputReader(opts, new HSoundcardReader<int16_t>(opts->GetInputDevice(), opts->GetInputSampleRate(), 1, H_SAMPLE_FORMAT_INT_16, BLOCKSIZE));
            break;
        case SIGNAL_GENERATOR:
            ...
            _inputReader = new BoomaInputReader(opts, new HSineGenerator<uint8_t>(opts->GetInputSampleRate(), opts->GetSignalGeneratorFrequency(), 200));
            break;
        case PCM_FILE:
            HLog("Initializing pcm file reader for input file %s", opts->GetPcmFile().c_str());
            _inputReader = new BoomaInputReader(opts, new HFileReader<int16_t>(opts->GetPcmFile()));
            break;
        case WAV_FILE:
            HLog("Initializing wav file reader for input file %s", opts->GetWavFile().c_str());
            _inputReader = new BoomaInputReader(opts, new HWavReader<int16_t>(opts->GetWavFile().c_str()));
            break;
        case SILENCE:
            HLog("Initializing nullreader");
            _inputReader = new BoomaInputReader(opts, new HNullReader<int16_t>());
            break;
        case RTLSDR:
            switch(opts->GetInputSourceDataType()) {
                case InputSourceDataType::IQ_INPUT_SOURCE_DATA_TYPE:
                    ...
                    _inputReader = new BoomaInputReader(opts, new HRtl2832Reader<uint8_t>(opts->GetInputDevice(), opts->GetInputSampleRate(), HRtl2832::MODE::IQ_SAMPLES, 0, _hardwareFrequency, BLOCKSIZE, 0, opts->GetRtlsdrCorrection()));
                    break;
                case InputSourceDataType::I_INPUT_SOURCE_DATA_TYPE:
                    ...
                    _inputReader = new BoomaInputReader(opts, new HRtl2832Reader<uint8_t>(opts->GetInputDevice(), opts->GetInputSampleRate(), HRtl2832::MODE::I_SAMPLES, 0, _hardwareFrequency, BLOCKSIZE, 0, opts->GetRtlsdrCorrection()));
                    break;
                case InputSourceDataType::Q_INPUT_SOURCE_DATA_TYPE:
                    ...
                    _inputReader = new BoomaInputReader(opts, new HRtl2832Reader<uint8_t>(opts->GetInputDevice(), opts->GetInputSampleRate(), HRtl2832::MODE::Q_SAMPLES, 0, _hardwareFrequency, BLOCKSIZE, 0, opts->GetRtlsdrCorrection()));
                    break;
                case InputSourceDataType::REAL_INPUT_SOURCE_DATA_TYPE:
                    ...
                    _inputReader = new BoomaInputReader(opts, new HRtl2832Reader<uint8_t>(opts->GetInputDevice(), opts->GetInputSampleRate(), HRtl2832::MODE::REAL_SAMPLES, 0, _hardwareFrequency, BLOCKSIZE, 0, opts->GetRtlsdrCorrection()));
                    break;
                default:
