...
bool CreateCwReceiverChain(ConfigOptions* configOptions) {
    ...
    // Todo: add buffer to Hardt
    HBuffer<int16_t> buffer(processor->Consumer(), BLOCKSIZE, 4);
    ...
    HHumFilter<int16_t> humFilter(buffer->Reader(), H_SAMPLE_RATE_48K, 50, 500, BLOCKSIZE);
    ...
    HGain<int16_t> volume(lowpass.Consumer(), 200, BLOCKSIZE);
...
//    HCustomWriter<HSignalLevelResult>* result = HCustomWriter<HSignalLevelResult>::Create(callback, level.Consumer());
    ...
    HFade<int16_t> fade(rxProcessor.Consumer(), 0, 1000, true, BLOCKSIZE);
