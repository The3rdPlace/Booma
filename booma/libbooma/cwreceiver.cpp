...
BoomaCwReceiver::BoomaCwReceiver(ConfigOptions* opts, int initialFrequency, int initialRfGain):
...
HWriterConsumer<int16_t>* BoomaCwReceiver::PreProcess(ConfigOptions* opts, HWriterConsumer<int16_t>* previous) {
    ...
    _highpass = new HBiQuadFilter<HHighpassBiQuad<int16_t>, int16_t>(_passthrough->Consumer(), 2000, opts->GetOutputSampleRate(), 0.7071f, 1, BLOCKSIZE, _highpassProbe);
    ...
    _gain = new HGain<int16_t>(_highpass->Consumer(), GetRfGain(), BLOCKSIZE, _gainProbe);

    // Add a combfilter to kill (more) 50 hz harmonics
    HLog("- Humfilter");
    _humfilterProbe = new HProbe<int16_t>("cwreceiver2_04_humfilter", _enableProbes);
    _humfilter = new HHumFilter<int16_t>(_gain->Consumer(), opts->GetOutputSampleRate(), 50, 1000, BLOCKSIZE, _humfilterProbe);
...
HWriterConsumer<int16_t>* BoomaCwReceiver::Receive(ConfigOptions* opts, HWriterConsumer<int16_t>* previous) {
    ...
    _ifMixer = new HMultiplier<int16_t>(_agc->Consumer(), opts->GetOutputSampleRate(), GetFrequency() - 6000 + offset, 10, BLOCKSIZE, _ifMixerProbe);
